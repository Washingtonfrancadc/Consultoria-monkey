...
function renderizarModoAluno() {
  ...
  if (dadosDoAluno.rotinasDieta.length === 0) {
    document.getElementById("conteudo-refeicoes-dinamicas").innerHTML =
      '<p style="text-align:center; padding:20px; color:var(--texto-mutado);">Nenhum protocolo ativo de dieta encontrado.</p>';
  } else {
    document.getElementById("conteudo-refeicoes-dinamicas").innerHTML =
        ...
        .map((dia) => {
          ...
          dia.refeicoes.forEach((ref) => {
            ref.alimentos.forEach((al) => {
              diaKcal += al.kcalTotal;
              diaCarbo += al.carboTotal;
              diaProt += al.protTotal;
              diaGord += al.gordTotal;
            });
          });

          let htmlRefeicoesInternas = dia.refeicoes
            .map((ref) => {
              let refKcal = 0,
                refCarbo = 0,
                refProt = 0,
                refGord = 0;
              let lines = ref.alimentos
                .map((al) => {
                            ...
                            <div class="corpo-recolhivel">
                              <div class="corpo-recolhivel-inner">
                                ...
                                <div class="resumo-macros">
                                    <div class="macro-box"><p>Total</p><div>${Math.round(diaKcal)} kcal</div></div>
...
function gerenciarIframeInjetado(exId, url) {
  ...
  if (linhaExistente) {
    ...
    if (wrapperAnitgo) {
      ...
      // Aguarda o fade out terminar antes de mover e carregar o novo GIF
      setTimeout(() => {
      ...
      }, 500); // Tempo exato da transição CSS do seu style
    }
...
async function enviarFeedbackCompleto() {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '<i class="fa-solid fa-spinner fa-spin"></i> Enviando Relatório...';
  ...
  for (let posicao in arquivosFotos) {
    if (arquivosFotos[posicao]) {
      try {
        ...
        urlsGeradas[posicao] =
          `${SUPABASE_URL}/storage/v1/object/public/fotos-evolucao/${nomeArquivo}`;
      } catch (uploadErr) {
  ...
  try {
    ...
    alert(
      "Boa, o relatório com as fotos foi enviado com sucesso! O gorila tá evoluindo! 🦍",
    );
    ...
    alert(
      "Ocorreu um erro ao registrar as informações no banco de dados. Tente novamente.",
    );
  } finally {
    btn.disabled = false;
    btn.innerHTML =
